...
#include "productid_files/board-omap3logic.h"
...
#define GPIO_I2C_GPIO_SCLK  5
#define GPIO_I2C_GPIO_SDATA 8
...
static void gpio_i2c_config_pins(void)
{
	...
	rc = gpio_request(GPIO_I2C_GPIO_SCLK, "gpio_i2c3_sclk");
	if (rc)
		printk("%s: gpio_request(gpio_i2c3_sclk) failed with %d\n", __FUNCTION__, rc);
	rc = gpio_request(GPIO_I2C_GPIO_SDATA, "gpio_i2c3_sdata");
	if (rc)
		printk("%s: gpio_request(gpio_i2c3_sdata) failed with %d\n", __FUNCTION__, rc);
...
static int send_packet(uint8_t *data, int len, uint8_t *rxbuf, int rxlen)
{
	...
	gpio_i2c_tx_stop();
	//printk(KERN_EMERG "rxbuf[0]:%2x rxbuf[1]:%2x rxbuf[2]:%2x rxbuf[3]:%2x rxbuf[4]:%2x rxbuf[5]:%2x rxbuf[6]:%2x\n", rxbuf[0], rxbuf[1], rxbuf[2], rxbuf[3], rxbuf[4], rxbuf[5], rxbuf[6]);
	return err;
}
...
static int header_version = -EINVAL;
...
#if 1
static int extract_part_number(void)
{
	char buf[32];
	int ret;
	u32 part_number;
...
int omap3logic_extract_model_number_revision(struct product_id_data *p, char *buf, int buflen)
{
	int i;
...
static int extract_product_id_mac_address(struct product_id_data *p, int position, unsigned char mac[6])
{
	...
	if (!omap3logic_is_product_data_valid())
	{
		printk("########## !omap3logic_is_product_data_valid");
		return -EINVAL;
	}
	//else
	//{
	//	printk("########## omap3logic_is_product_data_valid: %i", position);
	//}
...
//static int omap3logic_has_murata_wifi_module(void);
...
void valid_data_extract_dump(struct product_id_data *p)
{
	...
	ret = omap3logic_extract_model_number_revision(p, buf, sizeof(buf));
	if (!ret && DEBUG)
	{
		printk(KERN_INFO "Model Name   : %s\n", buf);
	}

	ret = omap3logic_extract_serial_number(p, buf, sizeof(buf));
	if (!ret && DEBUG)
	{
	...
	{
		printk(KERN_INFO "MAC Address: Retrievel failed\n");
	}
...
static int fetch_product_id_data(void)
{
	...
	if (set_user_zone(0) || read_user_zone((unsigned char *)&product_id_data.d.zone1, sizeof(product_id_data.d.zone1), 0x20)) {
		printk(KERN_ERR "failed!\n");
		err = -3;
	goto out;
	}

	if (set_user_zone(0) || read_user_zone((unsigned char *)&product_id_data.d.zone2, sizeof(product_id_data.d.zone2), 0x40)) {
		printk(KERN_ERR"failed!\n");
		err = -4;
		goto out;
	}
	...
	if(DEBUG)
	{
		printk(KERN_INFO "Production Data checksum: %i, Read: %i\n", checksum, product_id_data.checksum);
	}
	if (checksum != product_id_data.checksum && DEBUG) {
...
{
	unsigned char mac[6];
	extract_product_id_mac_address(&product_id_data, 0, mac);
...
{
	unsigned char mac[6];
        extract_product_id_mac_address(&product_id_data, 1, mac);
        return sprintf(buf, "%02x:%02x:%02x:%02x:%02x:%02x\n",
                        mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
}
...
static ssize_t product_id_show_zone2_data(struct class *class, struct class_attribute *attr, char *buf)
{
	int len = 0;
...
int omap3logic_create_old_product_id_sysfs(void)
{
	int i, rc;
...
int __init omap3logic_fetch_old_production_data(void)
{
	int valid;
...
EXPORT_SYMBOL(omap3logic_fetch_old_production_data);
subsys_initcall_sync(omap3logic_fetch_old_production_data);
